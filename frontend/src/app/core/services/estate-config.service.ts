...
export class PlatformConfigService {
  ...
  private get estateId(): string {
    ...
    if (!id)
      throw new Error(
        "No active estate — cannot call PlatformConfigService without an estate context.",
      );
    return id;
  }
   ...
   */
  getAll(): Observable<EstateConfigDTO[]> {
    return this.http.get<EstateConfigDTO[]>(`${this.base}/settings`);
  }

  /**
   * GET /api/v1/estates/{estateId}/config/settings/{key}
   */
  getOne(key: string): Observable<EstateConfigDTO> {
    return this.http.get<EstateConfigDTO>(`${this.base}/settings/${key}`);
  }
  ...
  ): Observable<EstateConfigDTO> {
    return this.http.put<EstateConfigDTO>(`${this.base}/settings/${key}`, req);
  }
   ...
   */
  bulkUpdate(req: BulkUpdateConfigRequest): Observable<EstateConfigDTO[]> {
  ...
  updateAssetTypeMapping(assetType: string, req: any): Observable<any> {
    return this.http.put<any>(
      `${this.base}/asset-type-mappings/${assetType}`,
      req,
    );
  }
}
