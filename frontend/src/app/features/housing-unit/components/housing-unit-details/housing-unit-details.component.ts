  ...
  template: `
    <div class="details-container" *ngIf="unit">
      ...
      <section class="info-section">
        ...
        <div class="info-grid">
          <div>
            <label>Unit Number</label><span>{{ unit.unitNumber }}</span>
          </div>
          <div>
            <label>Floor</label><span>{{ unit.floor }}</span>
          </div>
          <div *ngIf="unit.landingNumber">
            <label>Landing</label><span>{{ unit.landingNumber }}</span>
          </div>
          <div *ngIf="unit.totalSurface">
            <label>Total Surface</label><span>{{ unit.totalSurface }} m²</span>
          </div>
          <div>
            <label>Owner</label><span>{{ unit.ownerName ?? "—" }}</span>
          </div>
          <div *ngIf="unit.hasTerrace">
            <label>Terrace</label>
            <span
              >{{ unit.terraceSurface }} m² ·
              {{ unit.terraceOrientation }}</span
            >
          </div>
          <div *ngIf="unit.hasGarden">
            <label>Garden</label>
            <span
              >{{ unit.gardenSurface }} m² · {{ unit.gardenOrientation }}</span
            >
          </div>
        </div>
      </section>
      ...
      <section class="placeholder-section">
        ...
        <p class="placeholder-text">
          Room management will be available in UC003.
        </p>
      </section>
      ...
      <section class="placeholder-section">
        ...
        <p class="placeholder-text">
          PEB management will be available in UC004.
        </p>
      </section>
      ...
      <section class="placeholder-section">
        ...
        <p class="placeholder-text">
          Rent management will be available in UC005.
        </p>
      </section>
    ...
    <div class="dialog-overlay" *ngIf="showDeleteConfirm">
      <div class="dialog">
        ...
        <p>
          Unit <strong>{{ unit?.unitNumber }}</strong> will be permanently
          removed.
        </p>
