...
export class HousingUnitDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private route: ActivatedRoute, private router: Router, private housingUnitService: HousingUnitService) {}

  ngOnInit(): void {
    const id = +(this.route.snapshot.paramMap.get('id') ?? 0);
    if (id) { this.loading = true; this.housingUnitService.getUnitById(id).pipe(takeUntil(this.destroy$)).subscribe({ next: (u) => { this.unit = u; this.loading = false; }, error: () => { this.loading = false; } }); }
  }
  ...
  delete(): void {
    if (!this.unit) return;
    this.deleting = true;
    this.housingUnitService.deleteUnit(this.unit.id).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => { this.router.navigate(['/buildings', this.unit!.buildingId]); },
      error: (err) => { this.deleting = false; this.deleteError = err.error?.message ?? 'Failed to delete unit.'; }
    });
  }
}
