...
@Component({
    ...
    <section class="room-section">
      ...
      <div class="section-header">
        ...
        <div class="header-actions">
          <button class="btn btn-sm btn-primary" (click)="openAddForm()">Add Room</button>
          <button class="btn btn-sm btn-secondary" (click)="openQuickAdd()">Quick Add</button>
        </div>
      </div>
      ...
      <!-- Empty state -->
      <p *ngIf="!loading && rooms.length === 0 && !showAddForm && !showQuickAdd" class="empty-state">
        No rooms defined yet.
      </p>

      <!-- Rooms table -->
      <div *ngIf="!loading && rooms.length > 0 && !showAddForm && !showQuickAdd" class="table-wrapper">
        <table class="rooms-table">
          ...
          <tbody>
            <tr *ngFor="let room of rooms">
              ...
              <ng-container *ngIf="editingRoomId !== room.id">
                ...
                <td class="actions">
                  <button class="btn-icon" title="Edit" (click)="startEdit(room)">✏️</button>
                  <button class="btn-icon" title="Delete" (click)="requestDelete(room)">🗑️</button>
                </td>
              </ng-container>
              ...
              <ng-container *ngIf="editingRoomId === room.id">
                <td>
                  <select [formControl]="editForm.controls['roomType']" class="select-inline">
                    <option *ngFor="let t of allTypes" [value]="t">{{ roomLabel(t) }}</option>
                  </select>
                  <span class="field-error" *ngIf="editForm.controls['roomType'].invalid && editForm.controls['roomType'].touched">
                    Type is required
                  </span>
                </td>
                <td>
                  <input type="number" [formControl]="editForm.controls['approximateSurface']"
                         class="input-inline" min="0.01" max="999.99" step="0.01" />
                  <span class="field-error" *ngIf="editForm.controls['approximateSurface'].invalid && editForm.controls['approximateSurface'].touched">
                    Surface must be between 0.01 and 999.99
                  </span>
                </td>
                <td class="actions">
                  <button class="btn btn-sm btn-primary" (click)="saveEdit(room)" [disabled]="saving">
                    {{ saving ? '…' : 'Save' }}
                  </button>
          ...
          <tfoot>
            <tr class="total-row">
              ...
              <td colspan="2"><strong>{{ totalSurface | number:'1.2-2' }} m²</strong></td>
            </tr>
          </tfoot>
        </table>
      </div>
        ...
        <form [formGroup]="addForm" (ngSubmit)="saveAdd()">
          <div class="form-row">
            <div class="form-field">
              ...
              <select formControlName="roomType" [class.invalid]="isInvalidAdd('roomType')">
                <option value="">— Select type —</option>
                <option *ngFor="let t of allTypes" [value]="t">{{ roomLabel(t) }}</option>
              </select>
              <span class="field-error" *ngIf="isInvalidAdd('roomType')">Room type is required</span>
            </div>
            <div class="form-field">
              <label>Surface (m²) <span class="required">*</span></label>
              <input type="number" formControlName="approximateSurface"
                     [class.invalid]="isInvalidAdd('approximateSurface')"
                     min="0.01" max="999.99" step="0.01" placeholder="e.g. 15.5" />
              <span class="field-error" *ngIf="isInvalidAdd('approximateSurface')">
                Surface must be between 0.01 and 999.99 m²
              </span>
            </div>
          </div>
      ...
      <div *ngIf="showQuickAdd" class="form-card">
        ...
        <form [formGroup]="quickForm" (ngSubmit)="saveQuickAdd()">
          <table class="quick-table">
            ...
            <tbody>
              <tr *ngFor="let row of quickRows.controls; let i = index" [formGroupName]="i">
                <td>
                ...
                <td>
                  <input type="number" formControlName="approximateSurface"
                         ...
                         min="0.01" max="999.99" step="0.01" placeholder="0.00" />
                </td>
                <td>
                  <button type="button" class="btn-icon" title="Remove row"
                          (click)="removeQuickRow(i)" [disabled]="quickRows.length <= 1">
                    ✕
                  </button>
                </td>
              </tr>
          ...
          <div class="quick-footer">
            <button type="button" class="btn btn-secondary btn-sm"
                    (click)="addQuickRow()" [disabled]="quickRows.length >= 20">
              + Add Row
            </button>
  ...
  styles: [`
    ...
    .rooms-table th { background: #f5f5f5; font-weight: 600; }
    .total-row td { border-top: 2px solid #ccc; border-bottom: none; }
    ...
    .quick-table th { font-weight: 600; font-size: 0.85rem; color: #555; }
    .quick-table select, .quick-table input[type=number] {
      width: 100%;
      padding: 0.3rem 0.5rem;
      border: 1px solid #ccc;
      border-radius: 4px;
    }
    .quick-table .invalid { border-color: #c62828; }
    .quick-footer {
      display: flex;
      align-items: center;
      gap: 1rem;
    ...
    .row-count { font-size: 0.85rem; color: #888; }
    ...
    .form-card h4 { margin: 0 0 0.75rem 0; }
    .form-row {
      display: flex;
      gap: 1rem;
      flex-wrap: wrap;
    ...
    .form-field select.invalid, .form-field input.invalid { border-color: #c62828; }
    ...
    .loading { color: #555; padding: 0.5rem 0; }
    .required { color: #c62828; }
    .field-error { color: #c62828; font-size: 0.8rem; }
    .form-error { color: #c62828; font-size: 0.9rem; margin-bottom: 0.5rem; }
    .error-banner {
      background: #fdecea;
    ...
    .dialog h4 { margin: 0 0 0.75rem 0; }
    .dialog .warning { color: #c62828; font-size: 0.9rem; }
...
export class RoomSectionComponent implements OnInit, OnDestroy {
  ...
  readonly roomLabel = (t: RoomType | string) => ROOM_TYPE_LABELS[t as RoomType] ?? t;
  ...
  private buildForms(): void {
    this.addForm = this.fb.group({
      ...
      approximateSurface: [null, [Validators.required, Validators.min(0.01), Validators.max(999.99)]],
    });
    ...
    this.quickForm = this.fb.group({
      rows: this.fb.array([this.buildQuickRow(), this.buildQuickRow(), this.buildQuickRow()]),
    });
  }
  ...
  saveQuickAdd(): void {
    ...
    const validEntries: Array<{ roomType: RoomType; approximateSurface: number }> = [];
    let hasErrors = false;
    ...
    for (let i = 0; i < this.quickRows.length; i++) {
      ...
      if (!roomType || approximateSurface === null || approximateSurface <= 0 || approximateSurface >= 1000) {
        row.markAllAsTouched();
        hasErrors = true;
        continue;
      }

      validEntries.push({ roomType: roomType as RoomType, approximateSurface: Number(approximateSurface) });
    }

    if (hasErrors) {
      this.quickError = 'Some rows have invalid data. Please fix them or remove the rows.';
      return;
    }
