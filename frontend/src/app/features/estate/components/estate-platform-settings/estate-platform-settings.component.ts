...
export class EstatePlatformSettingsComponent implements OnInit, OnDestroy {
  ...
  loadValidityRules(): void {
    ...
    this.configService.getValidityRules()
  ...
  saveRule(): void {
    ...
    this.configService.addValidityRule(req)
      .pipe(takeUntil(this.destroy$), finalize(() => (this.ruleSaving = false)))
      .subscribe({
        next: () => {
          this.showAddRuleForm = false;
        ...
        error: (err) => {
          this.ruleError = err?.error?.message ?? 'Erreur lors de la sauvegarde.';
        },
      });
  }
  ...
  loadSubcategories(): void {
    this.subcategoryService.getAll().pipe(takeUntil(this.destroy$)).subscribe({
  ...
  loadAssetMappings(): void {
    ...
    this.configService.getAssetTypeMappings()
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (mappings) => {
  ...
  updateMapping(assetType: string, subcategoryId: number | null): void {
    ...
    this.configService.updateAssetTypeMapping(assetType, { subcategoryId })
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
        ...
        error: (err) => {
          this.mappingError = err?.error?.message ?? 'Erreur lors de la mise à jour.';
        },
      });
  }
  ...
  get generalConfigs():  PlatformConfigDTO[] { return this.configsFor(GENERAL_KEYS); }
  get alertConfigs():    PlatformConfigDTO[] { return this.configsFor(ALERT_KEYS); }
  get importConfigs():   PlatformConfigDTO[] { return this.configsFor(IMPORT_KEYS); }
  get boilerConfigs():   PlatformConfigDTO[] { return this.configsFor(BOILER_KEYS); }
