    ...
    <div class="detail-panel" *ngIf="row">
      ...
      <div class="panel-body">
        ...
        <div class="panel-section">
          ...
          <div
            *ngIf="row.suggestedSubcategory && !selectedSubcategoryId"
          ...
          >
            ...
            <optgroup
              *ngFor="let group of groupedSubcategories"
        ...
        <div class="panel-section">
          ...
          <div
            *ngIf="row.suggestedLease && !selectedLeaseId"
          ...
          >
            ...
            <option *ngFor="let b of buildings" [ngValue]="b.id">
              {{ b.name }}
            </option>
          </select>
...
export class ImportRowDetailPanelComponent implements OnInit, OnChanges {
  ...
  private syncFromRow(): void {
    ...
    if (targetLeaseId && this.allLeases.length) {
      // Leases loaded — filter by building and auto-select
      const lease = this.allLeases.find((l) => l.id === targetLeaseId);
      if (lease) {
        this.filterBuildingId = lease.buildingId;
        this.applyBuildingFilter();
        this.selectedLeaseId = targetLeaseId; // always select, whether enriched or suggested
      }
    } else if (this.row.suggestedLease?.buildingId) {
      // Leases not yet loaded — pre-fill building so filter is ready when leases arrive
  ...
  private loadLeases(): void {
    // Load ALL statuses — required for historical imports (FINISHED leases)
    ...
    this.leaseService.getAll(filters, 0, 500).subscribe((page) => {
      ...
      // Re-sync now that leases are available
  ...
  acceptLeaseSuggestion(): void {
    ...
    // 1. Pre-fill building filter so the lease appears in the dropdown
    this.filterBuildingId = suggested.buildingId ?? null;
    this.applyBuildingFilter();

    // 2. Select the lease — triggers full enrichment
