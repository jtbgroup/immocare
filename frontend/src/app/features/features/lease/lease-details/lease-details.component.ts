...
import { IndexationSectionComponent } from "../indexation-section/indexation-section.component";
...
@Component({
  ...
  imports: [
    ...
    RouterModule,
    FormsModule,
    TenantSectionComponent, // UC010 — point 4
    IndexationSectionComponent, // UC010 — point 4
  ],
...
export class LeaseDetailsComponent implements OnInit {
  ...
  private transition(target: string): void {
    ...
    this.leaseService
      .changeStatus(this.lease.id, { targetStatus: target as any })
      .subscribe({
        next: (updated) => {
          this.lease = updated;
        ...
        error: (err) => {
          this.statusError = err.error?.message || "Transition failed.";
          this.isTransitioning = false;
        },
      });
  }
