...
import {
  Lease, LeaseType, LEASE_TYPE_LABELS, DEFAULT_NOTICE_MONTHS, LEASE_DURATION_MONTHS,
  AddTenantRequest, PersonSummary
...
export class LeaseFormComponent implements OnInit {
  ...
  saveAsDraft(): void { this.submit(false); }
  saveAndActivate(): void { this.submit(true); }
  ...
  submit(activate: boolean): void {
    ...
    if (this.isEditMode) {
      this.leaseService.update(this.leaseId!, { ...value }).subscribe({
        ...
        error: err => { this.errorMessage = err.error?.message || 'Error saving lease.'; this.isSaving = false; }
      });
    } else {
      this.leaseService.create({ ...value, housingUnitId: this.unitId!, tenants }, activate).subscribe({
        next: lease => this.router.navigate(['/leases', lease.id]),
        error: err => { this.errorMessage = err.error?.message || 'Error creating lease.'; this.isSaving = false; }
      });
    }
  }
