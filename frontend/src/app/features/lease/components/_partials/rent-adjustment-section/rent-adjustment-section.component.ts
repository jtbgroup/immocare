...
import { AdjustRentRequest, Lease } from "../../../../../models/lease.model";
...
export class RentAdjustmentSectionComponent implements OnInit {
  ...
  get currentValue(): number {
    return this.req.field === "RENT"
      ? this.lease.monthlyRent
      : this.lease.monthlyCharges;
  }
  ...
  save(): void {
    ...
    if (
      !this.req.field ||
      this.req.newValue == null ||
      !this.req.reason ||
      !this.req.effectiveDate
    ) {
    ...
    this.isSaving = true;
    this.leaseService
      .adjustRent(this.lease.id, this.req as AdjustRentRequest)
      .subscribe({
        next: () => {
          this.showForm = false;
          this.isSaving = false;
