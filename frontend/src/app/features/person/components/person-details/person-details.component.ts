...
export class PersonDetailsComponent implements OnInit {
  ...
  ibanForm: SavePersonBankAccountRequest = {
    iban: "",
    label: "",
    primary: false,
  };
  ...
  doDelete(): void {
    ...
    this.personService.delete(this.person.id).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 409) {
          this.deleteError =
            err.error?.message || "This person cannot be deleted.";
  ...
  saveIban(): void {
    ...
    const obs = this.editingIban
      ? this.personBankAccountService.update(
          personId,
          this.editingIban.id,
          this.ibanForm,
        )
  ...
  deleteIban(id: number): void {
    ...
    this.personBankAccountService.delete(personId, id).subscribe({
      ...
      error: (err) =>
        (this.ibanError = err?.error?.message || "Failed to delete IBAN."),
    });
  }
