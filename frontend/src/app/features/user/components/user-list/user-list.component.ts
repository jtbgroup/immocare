...
export class UserListComponent implements OnInit {
  ...
  get pagedUsers(): User[] { const start = (this.currentPage - 1) * this.pageSize; return this.sortedUsers.slice(start, start + this.pageSize); }
  get endIndex(): number { return Math.min(this.currentPage * this.pageSize, this.users.length); }
  pages(): number[] { return Array.from({ length: this.totalPages }, (_, i) => i + 1); }
  goToPage(page: number): void { if (page >= 1 && page <= this.totalPages) { this.currentPage = page; } }
  navigateTo(user: User): void { this.router.navigate(['/users', user.id]); }
  create(): void { this.router.navigate(['/users/new']); }
  sortIcon(field: SortField): string { if (this.sortField !== field) return '↕'; return this.sortDir === 'asc' ? '↑' : '↓'; }
}
