# ImmoCare — UC005 Manage Persons — Implementation Prompt

I want to implement Use Case UC005 - Manage Persons for ImmoCare.
...
|-------|-------|----------|--------|
| UC004.001 | View Persons List | MUST HAVE | 2 |
| UC004.002 | Create Person | MUST HAVE | 3 |
| UC004.003 | Edit Person | MUST HAVE | 2 |
| UC004.004 | Delete Person | SHOULD HAVE | 2 |
| UC004.005 | Assign Person as Owner | MUST HAVE | 3 |
| UC004.006 | Person Picker (Search) | MUST HAVE | 2 |
...
3. `PersonSummaryDTO` — `{ id, fullName, city, nationalIdSuffix }` — used by person picker (UC004.006)
...
7. `PersonRepository`:
   ...
   - `List<Person> findTop10ByLastNameContainingIgnoreCaseOrFirstNameContainingIgnoreCaseOrNationalIdContainingIgnoreCase(String, String, String)` — picker (UC004.006)
...
| GET | /api/v1/persons/{id} | UC004.003 |
| GET | /api/v1/persons/search?q= | UC004.006 (picker) |
| POST | /api/v1/persons | UC004.002 |
| PUT | /api/v1/persons/{id} | UC004.003 |
| DELETE | /api/v1/persons/{id} | UC004.004 |

10. Update `BuildingController`/`HousingUnitController` — `ownerId` field now used (already in DTOs after UC006.004 migration)
...
14. `PersonListComponent` — table: Last Name, First Name, Email, GSM, City, Role badges (Owner=blue, Tenant=green); search bar; pagination (UC004.001)
15. `PersonFormComponent` — all fields; country defaults to Belgium (UC004.002, UC004.003)
16. `PersonDetailsComponent` — all fields; "Owned Buildings" section; "Owned Units" section; Edit + Delete buttons
17. `PersonPickerComponent` — autocomplete; min 2 chars; ≤300ms; "Create new person" shortcut (UC004.006)
18. Integrate `PersonPickerComponent` into `BuildingFormComponent` and `HousingUnitFormComponent` (replace old owner_name text input) — UC004.005
...
- [ ] All UC004.001–UC004.006 acceptance criteria verified
