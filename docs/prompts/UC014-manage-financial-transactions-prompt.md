...
| US084 | Import Transactions (CSV / PDF) | MUST HAVE |
...
**`TransactionStatisticsDTO`** — see UC014 use case document for full structure (unchanged).
...
**Other repositories** — `BankAccountRepository`, `TagCategoryRepository`, `TagSubcategoryRepository`, `ImportBatchRepository`, `AccountingMonthRuleRepository`, `TransactionSpecification` — unchanged from UC014 use case document.
...
// BR-UC014-02: direction=EXPENSE && leaseId!=null → throw TransactionValidationException
// BR-UC014-03: unitId set → load unit → set buildingId = unit.building.id
// BR-UC014-06: validate subcategory direction compatibility
// BR-UC014-14: for each asset link → resolve housingUnit and building server-side:
...
// BR-UC014-09: if building set on transaction, BOILER must belong to that building
// BR-UC014-15: sum of non-null asset link amounts must not exceed transaction.amount
// BR-UC014-17: if assetLinks non-empty → for each distinct assetType in links:
...
**`TransactionController`** — endpoints unchanged from UC014 use case document.
...
// unchanged from UC014 use case document except removal of counterpartyName everywhere.
```
...
All other components (`TransactionListComponent`, `TransactionFormComponent`, `TransactionDetailComponent`, `TransactionImportComponent`, `TransactionReviewComponent`, `TransactionDashboardComponent`, `TransactionSettingsComponent`) follow the UC014 use case document with these corrections:
...
- CSV export columns (US089): remove `counterparty_name` column. Updated order: `reference`, `external_reference`, `transaction_date`, `accounting_month`, `direction`, `amount`, `category`, `subcategory`, `bank_account_label`, `counterparty_account`, `description`, `building`, `housing_unit`, `lease_reference`, `status`, `source`.
...
| ID | Rule |
|---|---|
| BR-UC014-01 | amount always positive; direction carries sign semantics |
| BR-UC014-02 | lease_id only when direction = INCOME |
| BR-UC014-03 | housingUnitId set → buildingId auto-derived |
| BR-UC014-04 | RECONCILED transactions fully immutable |
| BR-UC014-05 | Deduplication by SHA-256 fingerprint |
| BR-UC014-06 | Subcategory direction must be compatible with transaction direction |
| BR-UC014-07 | Learning rule confidence += 1 on each confirmation |
| BR-UC014-08 | Accounting month rule priority: specific > generic > default 0 |
| BR-UC014-09 | BOILER asset link: boiler must belong to transaction's building (when building set) |
| BR-UC014-11 | accountingMonth stored as first day of month |
| BR-UC014-12 | Enriched import rows → CONFIRMED; others → DRAFT |
| BR-UC014-13 | Lease suggestion loads all lease statuses |
| BR-UC014-14 | housingUnitId and buildingId on asset links resolved server-side only |
| BR-UC014-15 | Sum of asset link amounts must not exceed transaction total |
| BR-UC014-16 | Asset links are optional even for EXPENSE transactions |
| BR-UC014-17 | Asset type mapping from platform_config pre-fills subcategory; admin can override |
| BR-UC014-18 | Valid tag_learning_rule match_field values: COUNTERPARTY_ACCOUNT, DESCRIPTION, ASSET_TYPE only |
| BR-UC014-19 | ASSET_TYPE learning rule: match_value = asset type name; confidence reinforced when suggestion accepted |
