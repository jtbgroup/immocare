# ImmoCare — UC014 Manage Fire Extinguishers — Implementation Prompt

I want to implement Use Case UC014 - Manage Fire Extinguishers for ImmoCare.
...
| Rule | Implementation |
|---|---|
| BR-UC014-01 | On create: `existsByBuildingIdAndIdentificationNumberIgnoreCase` → throw `FireExtinguisherDuplicateNumberException`. On update: use `existsByBuildingIdAndNumberIgnoreCaseExcluding`. |
| BR-UC014-02 | If `req.unitId()` not null: load `HousingUnit`, verify `unit.building.id == buildingId`, else throw `IllegalArgumentException("The specified unit does not belong to this building")` |
| BR-UC014-03 | `revisionDate` must not be after `LocalDate.now()`, else throw `IllegalArgumentException("Revision date cannot be in the future")` |
| BR-UC014-04 | Cascade on entity handles deletion of revisions — no extra logic needed |
...
|--------|------|------|----------|-------|
| GET | `/api/v1/buildings/{buildingId}/fire-extinguishers` | — | `List<FireExtinguisherDTO>` 200 | UC012.004 |
| GET | `/api/v1/fire-extinguishers/{id}` | — | `FireExtinguisherDTO` 200 | UC012.004 |
| POST | `/api/v1/buildings/{buildingId}/fire-extinguishers` | `SaveFireExtinguisherRequest` | `FireExtinguisherDTO` 201 | UC012.001 |
| PUT | `/api/v1/fire-extinguishers/{id}` | `SaveFireExtinguisherRequest` | `FireExtinguisherDTO` 200 | UC012.002 |
| DELETE | `/api/v1/fire-extinguishers/{id}` | — | 204 | UC012.003 |
| POST | `/api/v1/fire-extinguishers/{id}/revisions` | `AddRevisionRequest` | `FireExtinguisherDTO` 201 | UC012.005 |
| DELETE | `/api/v1/fire-extinguishers/{extId}/revisions/{revId}` | — | 204 | UC012.007 |

### GlobalExceptionHandler — add these handlers

```java
// ─── UC014 - Fire Extinguishers ───────────────────────────────────────────────
