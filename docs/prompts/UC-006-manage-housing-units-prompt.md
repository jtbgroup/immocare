...
|--------|----------|-------|
| GET | /api/v1/buildings/{buildingId}/units | UC006.004 |
| GET | /api/v1/units/{id} | UC006.004 |
| POST | /api/v1/units | UC006.001 |
| PUT | /api/v1/units/{id} | UC006.002, UC006.005, UC006.006 |
| DELETE | /api/v1/units/{id} | UC006.003 |
...
| Rule | Description |
|---|---|
| BR-UC006-01 | `unitNumber` unique within building (case-insensitive) |
| BR-UC006-04 | `hasTerrace = false` → clear `terraceSurface` and `terraceOrientation` |
| BR-UC006-05 | `hasGarden = false` → clear `gardenSurface` and `gardenOrientation` |
| BR-UC006-06 | Cannot delete unit with rooms; return `roomCount` in error |
| BR-UC006-07 | `hasTerrace = true` → `terraceSurface` required and must be > 0 |
| BR-UC006-08 | `hasGarden = true` → `gardenSurface` required and must be > 0 |
| BR-UC006-09 | `effectiveOwnerName` = unit.owner ?? building.owner |
