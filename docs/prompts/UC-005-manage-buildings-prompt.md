# ImmoCare — UC008 Manage Buildings — Implementation Prompt

I want to implement Use Case UC008 - Manage Buildings for ImmoCare.
...
|-------|-------|----------|--------|
| UC005.001 | Create Building | MUST HAVE | 5 |
| UC005.002 | Edit Building | MUST HAVE | 3 |
| UC005.003 | Delete Building | MUST HAVE | 3 |
| UC005.004 | View Buildings List | MUST HAVE | 3 |
| UC005.005 | Search Buildings | SHOULD HAVE | 2 |

---

## REFERENCE DOCUMENTS

- `docs/analysis/use-cases/UC006-manage-buildings.md` — flows, business rules, test scenarios
- `docs/analysis/user-stories/UC005.001-005` — acceptance criteria per story
...
> **Note**: `owner_id` is a FK to `person`. The old `owner_name` VARCHAR column has been removed (UC005 migration).
...
- Business rules:
  - BR-UC005-01: name, streetAddress, postalCode, city, country required
  - BR-UC005-03: cannot delete if housing units exist → `BuildingHasUnitsException`

### `BuildingController`

| Method | Endpoint | Story |
|--------|----------|-------|
| GET | /api/v1/buildings | UC005.004, UC005.005 |
| GET | /api/v1/buildings/{id} | UC005.004 |
| POST | /api/v1/buildings | UC005.001 |
| PUT | /api/v1/buildings/{id} | UC005.002 |
| DELETE | /api/v1/buildings/{id} | UC005.003 |
| GET | /api/v1/buildings/cities | UC005.004 |
...
### Components
- `BuildingListComponent` — list with search bar, city filter dropdown, sort by name/city, pagination (UC005.004, UC005.005)
- `BuildingFormComponent` — create/edit form, "Has Owner" toggle showing person picker, unsaved-changes guard (UC005.001, UC005.002)
- `BuildingDetailsComponent` — view all fields, "Edit", "Delete" buttons, embedded `HousingUnitListComponent` (UC005.003)
...
- [ ] All UC005.001–UC005.005 acceptance criteria verified manually
