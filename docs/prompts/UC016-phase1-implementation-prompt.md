# ImmoCare — UC016 Manage Estates — Phase 1 Implementation Prompt

I want to implement Use Case UC016 - Manage Estates (Phase 1 of 6) for ImmoCare.
...
|-------|-------|----------|--------|
| US092 | Create Estate | MUST HAVE | 3 |
| US093 | Edit Estate | MUST HAVE | 2 |
| US094 | Delete Estate | MUST HAVE | 2 |
| US095 | List All Estates | MUST HAVE | 2 |
| US096 | Assign First Manager to Estate | MUST HAVE | 1 |
| US097 | View Estate Members | MUST HAVE | 1 |
| US098 | Add Member to Estate | MUST HAVE | 2 |
| US099 | Edit Member Role | MUST HAVE | 2 |
| US100 | Remove Member from Estate | MUST HAVE | 2 |
| US101 | Select Active Estate | MUST HAVE | 3 |
| US102 | View Estate Dashboard | MUST HAVE | 2 |
| US103 | View My Estates | MUST HAVE | 1 |
| US104 | Enforce Estate-scoped Data Access | MUST HAVE | 3 |
...
```sql
-- Use case: UC016 — Manage Estates (Phase 1)
...
|--------|------|------|----------|-------|
| GET | `/` | — | `Page<EstateDTO>` 200 | US095 |
| POST | `/` | `CreateEstateRequest` | `EstateDTO` 201 | US092 |
| PUT | `/{id}` | `UpdateEstateRequest` | `EstateDTO` 200 | US093 |
| DELETE | `/{id}` | — | 204 | US094 |
...
|--------|------|------|----------|-------|
| GET | `/api/v1/estates/mine` | `isAuthenticated()` | `List<EstateSummaryDTO>` 200 | US103 |
| GET | `/api/v1/estates/{id}/dashboard` | `@security.isMemberOf(#id)` | `EstateDashboardDTO` 200 | US102 |
| GET | `/api/v1/estates/{id}/members` | `@security.isManagerOf(#id)` | `List<EstateMemberDTO>` 200 | US097 |
| POST | `/api/v1/estates/{id}/members` | `@security.isManagerOf(#id)` | `EstateMemberDTO` 201 | US098 |
| PATCH | `/api/v1/estates/{id}/members/{userId}` | `@security.isManagerOf(#id)` | `EstateMemberDTO` 200 | US099 |
| DELETE | `/api/v1/estates/{id}/members/{userId}` | `@security.isManagerOf(#id)` | 204 | US100 |
...
| Rule | Where |
|---|---|
| BR-UC016-01: Estate name unique case-insensitively | Backend: `existsByNameIgnoreCase` check |
| BR-UC016-02: At least one MANAGER at all times | Backend: `countByEstateIdAndRole('MANAGER') > 1` before remove/demote |
| BR-UC016-03: Cannot remove self | Backend: `EstateSelfOperationException`; Frontend: disable button on own row |
| BR-UC016-04: Cannot change own role | Backend: `EstateSelfOperationException`; Frontend: disable dropdown on own row |
| BR-UC016-05: PLATFORM_ADMIN accesses all estates without membership | Backend: `EstateSecurityService.isPlatformAdmin()` bypasses membership check |
| BR-UC016-06: estateId always explicit in URL | Backend: path variable `{id}` on all estate-scoped endpoints |
| BR-UC016-08: VIEWER cannot mutate | Frontend: `canEdit()` signal hides action buttons |
| BR-UC016-09: Cannot delete estate with buildings | Backend: buildingCount check (= 0 in Phase 1; activated in Phase 2) |
...
- [ ] PLATFORM_ADMIN can create, edit, delete, and list all estates (US092–US095)
- [ ] Creating an estate with `firstManagerId` automatically assigns MANAGER role (US096)
- [ ] Duplicate estate name (case-insensitive) → HTTP 409 (US092 AC3, US093 AC4)
- [ ] Cannot delete estate with buildings → HTTP 409 with buildingCount (US094 AC1) — returns 0 in Phase 1
- [ ] MANAGER can view, add, edit role, and remove members (US097–US100)
- [ ] Cannot remove self or last MANAGER → HTTP 409 (US099 AC2–AC3, US100 AC1–AC2)
- [ ] VIEWER cannot access members list → HTTP 403 (US097 AC3)
- [ ] `GET /api/v1/estates/mine` returns correct estates and roles (US103)
- [ ] Dashboard endpoint returns estate name with all counts = 0 (US102)
- [ ] Estate selector shown on login when user has multiple estates (US101 AC2)
- [ ] Auto-redirect when user has exactly one estate (US101 AC1)
- [ ] `ActiveEstateService` signals update correctly on estate selection and switch (US101)
- [ ] Estate name visible in app header with "Switch" button (US101 AC4)
- [ ] `EstateSecurityService` correctly enforces PLATFORM_ADMIN bypass and membership checks (US104)
