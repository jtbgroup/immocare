# User Story UC015.002: Create Transaction Manually
...
**AC3:** `accounting_month` is auto-proposed on page load and whenever `subcategory` or `counterparty_account` changes, using accounting month rules (BR-UC015-08). Proposed value shown pre-filled; admin can override freely.
...
**AC7:** Housing unit dropdown populated only after a building is selected. Selecting a unit automatically sets `building_id` server-side (BR-UC015-03).
...
**AC9:** Lease link with direction = EXPENSE → validation error "Lease link is only allowed for income transactions." (BR-UC015-02).
