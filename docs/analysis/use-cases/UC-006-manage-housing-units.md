...
**Acceptance Criteria:**
- AC1: All fields from UC006.001 can be updated except `buildingId`.
...
### UC006.004 — View Housing Unit Details
...
### UC006.005 — Add Terrace to Housing Unit
...
### UC006.006 — Add Garden to Housing Unit
...
7. Controller: `HousingUnitController` — endpoints as per UC006.001–UC006.004. No @RequestMapping prefix; use full paths on each method. @PreAuthorize not required (security configured globally) but add if consistent with existing code.
...
3. Components (standalone):
   ...
   - `HousingUnitDetailsComponent` — shows all fields grouped in cards. Hosts sub-components: RoomSectionComponent (UC004), PebSectionComponent (UC005), RentSectionComponent (UC006), MeterSectionComponent (UC009), LeaseSectionComponent (UC011). Edit and delete buttons in header.
...
Note: UC006.005 (Add Terrace) and UC006.006 (Add Garden) are implemented via the same edit form as UC006.002. No dedicated endpoint — all handled by PUT /api/v1/units/{id}.
```
...
- [ ] All UC006.001–UC006.006 acceptance criteria verified manually
