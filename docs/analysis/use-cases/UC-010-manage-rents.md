...
### UC010.004 — Track Rent Increases Over Time
...
### UC010.005 — Add Notes to Rent Changes
...
| ID | Rule |
|---|---|
| BR-UC006-01 | `monthlyRent` must be > 0 |
| BR-UC006-02 | `effectiveFrom` cannot be more than 1 year in the future |
| BR-UC006-03 | Only one record per unit can have `effectiveTo = NULL` (the current rent) |
| BR-UC006-04 | Inserting a record automatically recalculates adjacent `effectiveTo` values |
| BR-UC006-05 | Deleting a record automatically restores the previous record's `effectiveTo` |
...
```
Generate the complete Spring Boot + Angular implementation for UC006 — Manage Rents in the ImmoCare application.
