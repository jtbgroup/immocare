# UC014 — Manage Platform Configuration
...
|---|---|---|---|
| UC013.001 | View Platform Settings | MUST HAVE | 2 |
| UC013.002 | Add Boiler Service Validity Rule | MUST HAVE | 3 |
| UC013.003 | View Boiler Service Validity Rules History | MUST HAVE | 2 |
| UC013.004 | Update General Alert Settings | MUST HAVE | 2 |
| UC012.001 | Manage Asset Type to Subcategory Mappings | MUST HAVE | 2 |
...
### View Platform Settings (UC013.001)
...
### Add Boiler Service Validity Rule (UC013.002)
...
### View Boiler Service Validity Rules History (UC013.003)
...
### Update General Alert Settings (UC013.004)
...
### Manage Asset Type to Subcategory Mappings (UC012.001)
...
| ID | Rule |
|---|---|
| BR-UC014-01 | Validity rules are append-only (no edit, no delete) |
| BR-UC014-02 | valid_from must be unique across all rules |
| BR-UC014-03 | To find the rule applicable to a service_date: SELECT rule WHERE valid_from ≤ service_date ORDER BY valid_from DESC LIMIT 1 |
| BR-UC014-04 | Adding a new rule does NOT retroactively recalculate existing boiler_service.valid_until values |
| BR-UC014-05 | Alert threshold must be a positive integer (months) |
| BR-UC014-06 | platform_config values are stored as VARCHAR and cast by value_type |
| BR-UC014-07 | Asset type mappings reference a subcategory whose direction is EXPENSE or BOTH; INCOME subcategories cannot be mapped to an asset type |
| BR-UC014-08 | Asset type mappings are applied as suggestions only — the admin can always override the pre-filled subcategory on a transaction |
| BR-UC014-09 | Each asset type (BOILER, FIRE_EXTINGUISHER, METER) has at most one mapped subcategory |
...
|---|---|---|
| GET | /api/v1/config/boiler-validity-rules | UC013.003 |
| POST | /api/v1/config/boiler-validity-rules | UC013.002 |
| GET | /api/v1/config/settings | UC013.001 + UC013.004 |
| PUT | /api/v1/config/settings/{key} | UC013.004 |
| GET | /api/v1/config/asset-type-mappings | UC012.001 |
| PUT | /api/v1/config/asset-type-mappings/{assetType} | UC012.001 |
