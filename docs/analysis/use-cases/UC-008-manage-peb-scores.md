...
### UC008.003 — Check PEB Certificate Validity
...
| ID | Rule |
|---|---|
| BR-UC005-01 | PEB scores are append-only — no update or delete endpoints |
| BR-UC005-02 | `isCurrent` = score with the most recent `scoreDate` |
| BR-UC005-03 | `scoreDate` cannot be in the future |
| BR-UC005-04 | `validUntil`, if provided, must be after `scoreDate` |
| BR-UC005-05 | Multiple scores on the same date are allowed (correction use case) |
| BR-UC005-06 | `status = EXPIRING_SOON` when `validUntil` is within 3 months of today |
| BR-UC005-07 | `status = EXPIRED` when `validUntil` is in the past |
...
```
Generate the complete Spring Boot + Angular implementation for UC005 — Manage PEB Scores in the ImmoCare application.
...
User Stories: UC008.001 (Add PEB Score), UC008.002 (View History), UC008.003 (Certificate Validity), UC008.004 (Track Improvements)
...
3. Component: `PebSectionComponent` (standalone, [unitId] input, inside HousingUnitDetailsComponent)
   - Shows current score as a colored badge with validity status (VALID / EXPIRING_SOON / EXPIRED) — UC008.003
   - "View History" link opens history table (UC008.002): columns Score, Date, Certificate, Valid Until, Status, Improvement indicator
   - Improvement summary shown if ≥ 2 records: "Improved by X grades over Y years" — UC008.004
   - Inline add form: score picker (color-coded), date (max today), certificate number (optional), valid until (optional) — UC008.001

Business rules to enforce in frontend:
- scoreDate: today's date as max (BR-UC005-03)
- validUntil: must be after scoreDate (BR-UC005-04)
- Display EXPIRING_SOON as orange badge, EXPIRED as red badge (UC008.003)
- Per-row improvement indicator in history: ↑ green / ↓ red / − grey (UC008.004)
```
