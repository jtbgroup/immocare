...
### UC002.004 — Change User Password
...
| ID | Rule |
|---|---|
| BR-UC008-01 | Only ADMIN role exists in Phase 1 |
| BR-UC008-02 | `username` must be unique (case-insensitive) |
| BR-UC008-03 | `email` must be unique (case-insensitive) |
| BR-UC008-04 | Password: ≥ 8 chars, ≥ 1 upper, ≥ 1 lower, ≥ 1 digit |
| BR-UC008-05 | `password` must equal `confirmPassword` |
| BR-UC008-06 | Password stored as BCrypt(12) hash |
| BR-UC008-07 | An admin cannot delete their own account |
| BR-UC008-08 | `UpdateUserRequest` does not update the password |
...
```
Generate the complete Spring Boot + Angular implementation for UC008 — Manage Users in the ImmoCare application.
...
6. Service: `UserService` — getAllUsers(), getUserById(id), createUser(req), updateUser(id, req), changePassword(id, req), deleteUser(id, currentUserId). Enforce all BR-UC008-xx. Password regex: ^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$
7. Controller: `UserController` — @RequestMapping("/api/v1/users"), all endpoints as per UC004.007–UC002.005. Use @AuthenticationPrincipal AppUser currentUser on DELETE to get current user id.
...
- BR-UC008-04/05: password complexity regex + confirm match validator
- BR-UC008-07: hide delete button when row.id === currentUser.id
