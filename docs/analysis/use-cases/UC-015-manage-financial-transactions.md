# UC015 — Manage Financial Transactions
...
|---|---|---|---|
| UC015.001 | View Transaction List | MUST HAVE | 3 |
| UC015.002 | Create Transaction Manually | MUST HAVE | 5 |
| UC015.003 | Edit Transaction | MUST HAVE | 3 |
| UC015.004 | Delete Transaction | MUST HAVE | 2 |
| UC015.005 | Classify Transaction (Category / Subcategory) | MUST HAVE | 3 |
| UC015.006 | Link Transaction to Asset(s) | SHOULD HAVE | 3 |
| UC015.007 | Import Transactions (CSV / PDF) | MUST HAVE | 8 |
| UC015.008 | Review and Confirm Imported Transactions | MUST HAVE | 5 |
| UC015.009 | Manage Tag Catalog (Categories & Subcategories) | MUST HAVE | 3 |
| UC015.010 | Manage Bank Account Catalog | MUST HAVE | 2 |
| UC015.011 | View Financial Summary and Statistics | SHOULD HAVE | 5 |
| UC015.012 | Export Transactions to CSV | SHOULD HAVE | 2 |
...
### Import Flow (3 steps)
1. **Form** — admin selects a parser (from UC016 registry) and uploads a file.
...
### Lease Suggestion
During preview and import, `counterparty_account` is matched against known person IBANs (UC004_ESTATE_PLACEHOLDER). If a match is found, the most suitable lease is suggested.
...
| ID | Rule |
|---|---|
| BR-UC015-01 | `amount` is always stored positive; `direction` carries the sign semantics |
| BR-UC015-02 | `lease_id` may only be set when `direction = INCOME` |
| BR-UC015-03 | When `housing_unit_id` is set on the transaction, `building_id` is automatically derived from the unit's building |
| BR-UC015-04 | RECONCILED transactions are fully immutable: no edit, no delete, no reclassification |
| BR-UC015-05 | Deduplication is fingerprint-based (SHA-256). Rows with a fingerprint already present in `import_fingerprint` are skipped |
| BR-UC015-06 | A subcategory with `direction = INCOME` cannot be applied to an EXPENSE transaction and vice versa; BOTH is always allowed |
| BR-UC015-07 | Tag learning rule confidence increments by 1 on each admin confirmation of a subcategory for a given match_field + match_value |
| BR-UC015-08 | Accounting month rule priority: specific (subcategory + counterparty_account) > generic (subcategory only) > default offset 0 |
| BR-UC015-09 | Asset link of type BOILER: when a building is set on the transaction, the boiler must belong to a unit of that building |
| BR-UC015-10 | Import requires a registered active parser (UC016); file extension must match parser format |
| BR-UC015-11 | `accounting_month` stored as first day of month (e.g. `2026-02-01`); displayed as month/year only |
| BR-UC015-12 | Enriched rows (matched fingerprint in enrichment payload) saved as CONFIRMED; all other imported rows saved as DRAFT |
| BR-UC015-13 | Lease suggestion during import loads leases of all statuses (DRAFT/ACTIVE/FINISHED/CANCELLED) to support historical imports |
| BR-UC015-14 | `housing_unit_id` and `building_id` on `transaction_asset_link` are resolved server-side from the device relationship and are never provided by the client |
| BR-UC015-15 | The sum of partial amounts on asset links must not exceed the transaction total amount |
| BR-UC015-16 | Asset links are optional even for EXPENSE transactions — a transaction may have no asset links |
| BR-UC015-17 | When an asset link is added, if platform config contains a mapping for the asset type (`asset.type.subcategory.mapping.*`), the transaction subcategory is pre-filled with the mapped value. The admin can override freely |
| BR-UC015-18 | `COUNTERPARTY_NAME` is not a valid `match_field` in `tag_learning_rule`. Valid values are: COUNTERPARTY_ACCOUNT, DESCRIPTION, ASSET_TYPE |
| BR-UC015-19 | A `tag_learning_rule` with `match_field = ASSET_TYPE` uses the asset type name as `match_value` (BOILER, FIRE_EXTINGUISHER, METER). Its confidence is reinforced each time an admin accepts a subcategory suggestion triggered by that asset type |
