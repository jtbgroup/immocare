...
| ID | Rule |
|---|---|
| BR-UC004-01 | `approximateSurface` must be > 0 and < 1000 |
| BR-UC004-02 | `roomType` must be a valid enum value |
| BR-UC004-03 | A housing unit must exist before rooms can be added |
| BR-UC004-04 | Batch creation is all-or-nothing (transactional) |
...
```
Generate the complete Spring Boot + Angular implementation for UC004 — Manage Rooms in the ImmoCare application.
...
7. Controller: `RoomController` — @RequestMapping("/api/v1/housing-units/{unitId}/rooms"), all endpoints as per UC006.005–UC007.003
