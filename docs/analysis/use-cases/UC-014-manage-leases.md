...
### UC014.001 — View Lease for Housing Unit
...
### UC014.008 — View Indexation History
...
### UC014.010 — Remove Tenant from Lease
...
| ID | Rule |
|---|---|
| BR-UC011-01 | Only one ACTIVE or DRAFT lease per unit at a time |
| BR-UC011-02 | At least one PRIMARY tenant required on create |
| BR-UC011-03 | Only DRAFT/ACTIVE leases are editable |
| BR-UC011-04 | Status transitions follow the state machine |
| BR-UC011-05 | Rent adjustments only on ACTIVE leases |
| BR-UC011-06 | `totalRent = monthlyRent + monthlyCharges` (computed) |
...
```
Generate the complete Spring Boot + Angular implementation for UC011 — Manage Leases in the ImmoCare application.
...
- BR-UC011-01: navigate away with error if unit already has ACTIVE/DRAFT lease
- BR-UC011-02: disable submit if no PRIMARY tenant
- BR-UC011-03: disable Edit button on FINISHED/CANCELLED leases
- BR-UC011-04: show only valid transitions in status change dropdown
