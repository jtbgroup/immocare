...
CREATE TABLE meter (
    ...
    CONSTRAINT chk_meter_type CHECK (
        type IN ('WATER', 'GAS', 'ELECTRICITY')
    ),
    CONSTRAINT chk_meter_owner_type CHECK (
        owner_type IN ('HOUSING_UNIT', 'BUILDING')
    ),
    CONSTRAINT chk_meter_end_after_start CHECK (
        end_date IS NULL
...
CREATE INDEX idx_meter_owner_history ON meter (
    owner_type,
    owner_id,
    start_date DESC
);
...
COMMENT ON
TABLE meter IS 'Append-only utility meter history (WATER, GAS, ELECTRICITY) per housing unit or building';
