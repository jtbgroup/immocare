...
INSERT INTO platform_config (config_key, config_value, description) VALUES
    ('csv.import.delimiter',                       ';',          'CSV column delimiter'),
    ('csv.import.date_format',                     'dd/MM/yyyy', 'Date format in CSV'),
    ('csv.import.skip_header_rows',                '1',          'Number of header rows to skip'),
    ('csv.import.col.date',                        '0',          'Column index for transaction date'),
    ('csv.import.col.amount',                      '1',          'Column index for amount (negative = EXPENSE)'),
    ('csv.import.col.description',                 '2',          'Column index for description / communication'),
    ('csv.import.col.counterparty_name',           '3',          'Column index for counterparty name'),
    ('csv.import.col.counterparty_account',        '4',          'Column index for counterparty IBAN'),
    ('csv.import.col.external_reference',          '5',          'Column index for bank transaction reference'),
    ('csv.import.col.bank_account',                '6',          'Column index for own bank account IBAN'),
    ('csv.import.col.value_date',                  '-1',         'Column index for value date (-1 = absent)'),
    ('csv.import.suggestion.confidence.threshold', '3',          'Min confidence to show tag suggestion')
