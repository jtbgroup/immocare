...
/**
 * DTOs for UC004_ESTATE_PLACEHOLDER Phase 5 — per-estate Platform Configuration
 * and Boiler
 * Validity Rules.
 * Replaces the global PlatformConfigDTOs for estate-scoped operations.
 */
public final class EstateConfigDTOs {

        public EstateConfigDTOs() {
        }

        /** Response: one config entry. */
        public record EstateConfigDTO(
                        UUID estateId,
                        String configKey,
                        String configValue,
                        String valueType,
        ...
        public record UpdateEstateConfigRequest(
        ...
        public static final List<EstateConfigSeed> DEFAULT_CONFIG = List.of(
                        new EstateConfigSeed(KEY_BOILER_ALERT_THRESHOLD_MONTHS, "3", "INTEGER",
                                        "Months before service expiry to display a warning alert"),
                        new EstateConfigSeed(KEY_ASSET_MAPPING_PREFIX + "BOILER", "", "STRING",
                                        "Subcategory ID to pre-fill when a BOILER asset link is added (empty = no mapping)"),
                        new EstateConfigSeed(KEY_ASSET_MAPPING_PREFIX + "FIRE_EXTINGUISHER", "", "STRING",
                                        "Subcategory ID to pre-fill when a FIRE_EXTINGUISHER asset link is added"),
                        new EstateConfigSeed(KEY_ASSET_MAPPING_PREFIX + "METER", "", "STRING",
                                        "Subcategory ID to pre-fill when a METER asset link is added"),
                        new EstateConfigSeed(KEY_CSV_DELIMITER, ";", "STRING",
                                        "CSV column delimiter"),
                        new EstateConfigSeed(KEY_CSV_DATE_FORMAT, "dd/MM/yyyy", "STRING",
                                        "Date format in CSV"),
                        new EstateConfigSeed(KEY_CSV_SKIP_HEADER_ROWS, "1", "INTEGER",
                                        "Header rows to skip"),
                        new EstateConfigSeed(KEY_CSV_COL_DATE, "0", "INTEGER",
                                        "Column index for date"),
                        new EstateConfigSeed(KEY_CSV_COL_AMOUNT, "1", "INTEGER",
                                        "Column index for amount"),
                        new EstateConfigSeed(KEY_CSV_COL_DESCRIPTION, "2", "INTEGER",
                                        "Column index for description"),
                        new EstateConfigSeed(KEY_CSV_COL_COUNTERPARTY_ACCOUNT, "3", "INTEGER",
                                        "Column index for counterparty IBAN"),
                        new EstateConfigSeed(KEY_CSV_COL_EXTERNAL_REFERENCE, "4", "INTEGER",
                                        "Column index for bank reference"),
                        new EstateConfigSeed(KEY_CSV_COL_BANK_ACCOUNT, "5", "INTEGER",
                                        "Column index for own IBAN"),
                        new EstateConfigSeed(KEY_CSV_COL_VALUE_DATE, "-1", "INTEGER",
                                        "Column index for value date (-1 = absent)"),
                        new EstateConfigSeed(KEY_CSV_SUGGESTION_CONFIDENCE, "3", "INTEGER",
