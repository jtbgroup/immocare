 ...
 * UC004_ESTATE_PLACEHOLDER Phase 5: loadMappingConfig() now accepts an estateId
 * and reads
...
public class CsvImportService {
         ...
         * UC004_ESTATE_PLACEHOLDER Phase 5: reads from estate config rather than global
         * keys.
         */
        ...
        public ImportBatchResultDTO importBatch(UUID estateId, List<ParsedCsvRow> rows, AppUser currentUser) {
                ...
                // Resolve estate once — used for tx, batch, and reference generation
                ...
                for (ParsedCsvRow row : rows) {
                        if (row.parseError() != null) {
                                errorCount++;
                                errors.add(new ImportBatchResultDTO.RowError(row.rowNumber(), row.rawLine(),
                                                row.parseError()));
                                continue;
                        }
                        ...
                        try {
                                ...
                                if (row.bankAccountIban() != null && !row.bankAccountIban().isBlank()) {
                                        bankAccountRepository
                                                        .findByEstateIdAndAccountNumber(estateId, row.bankAccountIban())
                                                        .ifPresent(tx::setBankAccount);
                                }

                                // Suggest subcategory
                                List<SubcategorySuggestionDTO> suggestions = learningService.suggestSubcategory(
                                                row.counterpartyAccount(), row.description(), row.direction(),
                                                minConfidence);
                                ...
                                // Generate estate-scoped reference: TXN-YYYY-NNNNN
                                int year = row.transactionDate().getYear();
                                String prefix = "TXN-" + year + "-";
                                int seq = transactionRepository.nextSequenceForYear(prefix + "%", estateId);
                                tx.setReference(prefix + String.format("%05d", seq));
                        ...
                        if (covering.size() == 1) {
                                ...
                                best = leases.stream()
                                                .min(Comparator.comparingLong(
                                                                l -> Math.abs(ChronoUnit.DAYS.between(l.getEndDate(),
                                                                                transactionDate))))
                                                .orElse(leases.get(0));
                        }
