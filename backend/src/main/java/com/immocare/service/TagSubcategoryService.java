...
public class TagSubcategoryService {
    ...
    public List<TagSubcategoryDTO> getAll(UUID estateId, Long categoryId) {
        List<TagSubcategory> list = categoryId != null
                ? tagSubcategoryRepository.findByCategory_EstateIdAndCategoryIdOrderByNameAsc(estateId, categoryId)
                : tagSubcategoryRepository.findByCategory_EstateIdOrderByCategory_NameAscNameAsc(estateId);
        return list.stream().map(this::toDTO).toList();
    }

    @Transactional
    public TagSubcategoryDTO create(UUID estateId, SaveTagSubcategoryRequest req) {
        TagCategory category = verifyCategoryBelongsToEstate(estateId, req.categoryId());

        if (tagSubcategoryRepository.existsByCategory_EstateIdAndCategoryIdAndNameIgnoreCase(
                estateId, req.categoryId(), req.name())) {
    ...
    @Transactional
    public TagSubcategoryDTO update(UUID estateId, Long id, SaveTagSubcategoryRequest req) {
        verifySubcategoryBelongsToEstate(estateId, id);
        ...
        if (tagSubcategoryRepository.existsByCategory_EstateIdAndCategoryIdAndNameIgnoreCaseAndIdNot(
                estateId, req.categoryId(), req.name(), id)) {
            throw new IllegalArgumentException("A subcategory with this name already exists in the category.");
        }
    ...
    /**
     * Verifies the category belongs to the estate and returns it.
     */
    private TagCategory verifyCategoryBelongsToEstate(UUID estateId, Long categoryId) {
        TagCategory category = tagCategoryRepository.findById(categoryId)
    ...
    /**
     * Verifies the subcategory belongs to a category in the given estate.
     */
    private void verifySubcategoryBelongsToEstate(UUID estateId, Long subcategoryId) {
        if (!tagSubcategoryRepository.existsById(subcategoryId)) {
            throw new SubcategoryNotFoundException("Subcategory not found: " + subcategoryId);
        }
